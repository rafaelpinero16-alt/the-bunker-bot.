...
async def is_user_creator(bot: Bot, chat_id: int, user_id: int) -> bool:
    """Valida si el usuario ostenta el rango máximo de Dueño / Creador del grupo."""
    try:
...
TEXTS = {
    "en": {
        ...
        "owner_only": "⛔ <b>Access Denied:</b> Only the Community Owner can alter acoustic protocols.\n\n🛡️ <i>Cloud Media Management</i>",
    ...
    "es": {
        ...
        "owner_only": "⛔ <b>Acceso Denegado:</b> Solo el Dueño de la comunidad puede alterar los protocolos acústicos.\n\n🛡️ <i>Cloud Media Management</i>",
...
# ==========================================================
# 🚀 COMANDOS Y CALLBACKS DEL ECOSISTEMA
# ==========================================================
@router.message(Command("status_bc", "statusvc"))
async def cmd_status_bc(message: Message):
    """Muestra la telemetría del ecosistema dentro del grupo."""
    ...
    keyboard = build_radar_markup(chat_id, lang, in_private=False)
    bot_msg = await message.answer(status_text, reply_markup=keyboard, parse_mode="HTML")
    asyncio.create_task(auto_delete_pair(message, bot_msg, delay=35))
...
async def cmd_cams(message: Message):
    """Auditoría de calidad y estabilidad de transmisiones en vivo."""
    lang = get_lang(message.from_user.language_code)
    if message.chat.type == "private":
...
async def cmd_autolower(message: Message):
    """Alterna el modo AutoLower. Protocolo reservado exclusivamente al Dueño."""
    lang = get_lang(message.from_user.language_code)
    t = TEXTS[lang]
    ...
    if not await is_user_creator(message.bot, message.chat.id, message.from_user.id):
        try:
...
async def cmd_mic_vip(message: Message):
    """Promoción de acceso al Pase VIP de Micrófono."""
    lang = get_lang(message.from_user.language_code)
    t = TEXTS[lang]
    chat_id = message.chat.id
    ...
    asyncio.create_task(auto_delete_pair(message, promo_msg, delay=45))