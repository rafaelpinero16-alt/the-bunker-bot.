...
from database.database import (
    ...
    # 🚨 ULTRA PRO — Panel de Élite, Módulos Corporativos & Centinela de IA
    ...
    get_ai_sentinel_config, set_ai_sentinel_config,
...
async def verify_admin_privileges(callback: CallbackQuery, bot: Bot, chat_id: int) -> bool:
    """Return whether the callback sender is an administrator of the chat."""
    ...
    try:
        ...
        logging.warning("No se pudieron verificar los privilegios en %s: %s", chat_id, exc)
        allowed = False

    if not allowed:
        try:
            await callback.answer(
                "❌ Solo los administradores pueden usar este panel.",
                show_alert=True,
            )
        except Exception:
            pass
...
def clear_user_states(bot_id: int, user_id: int) -> None:
    """Clear every pending private-chat flow for one user and bot."""
    keys = {
        (bot_id, user_id),
        f"{bot_id}:{user_id}",
        user_id,
    }
    for states in ALL_STATE_DICTS:
        for key in tuple(states):
            if key in keys:
                states.pop(key, None)
                continue
            # Some legacy state entries use a nested ``bot_id -> user_id`` map.
...
CHAT_KIND_CACHE: dict = {}
CHANNEL_SYNC_CACHE: dict = {}
_FALLBACK_LAST_REPLY: dict = {}


async def registry_upsert_channel(user_id: int, channel_id: int, title: str) -> None:
    """Persist a synced channel without blocking the event loop."""
    def _sync() -> None:
        with get_db_connection() as conn:
            conn.execute(
                """
                INSERT INTO user_channels (user_id, channel_id, title)
                VALUES (?, ?, ?)
                ON CONFLICT(user_id, channel_id) DO UPDATE SET title = excluded.title
                """,
                (user_id, channel_id, title or str(channel_id)),
            )
            conn.commit()
...
async def resolve_chat_kind(bot: Bot, chat_id: int) -> str:
    """Return ``c`` for channels and ``g`` for every other supported chat."""
    ...
    try:
        ...
        kind = "c" if chat.type == "channel" else "g"
    except Exception as exc:
        logging.warning("Could not resolve chat kind for %s: %s", chat_id, exc)
        kind = "g"
...
def user_lang(user) -> str:
    """Return the supported interface language for a Telegram user."""
    return "en" if getattr(user, "language_code", "") == "en" else "es"


async def verify_admin_privileges_msg(message: Message, bot: Bot, chat_id: int) -> bool:
    """Check whether the requesting user can manage the target chat."""
    ...
    try:
        ...
        logging.warning(
            "Privilege check failed for user %s in chat %s: %s",
            user.id,
            chat_id,
            exc,
        )

    await message.answer(TEXTS.get(user_lang(user), TEXTS["es"])["owner_only_alert"])
    return False
...
TEXTS = {
    "en": {
        ...
        "info_main": (
            ...
            "<b>The Bunker Bot</b>\n"
            "• <b>Version:</b> 6.2 (AI Sentinel Core)\n"
        ...
        "info_mod_groups": (
            ...
            "🔐 <b>Captcha Customs Pro:</b> Every new member faces a private DM challenge before gaining access.\n"
            "🔒 <b>Granular Locks:</b> Independent switches let you restrict media, links, stickers and commands.\n"
            "🚫 <b>Anti-Spam Shield:</b> Continuously scans the chat for forwarded messages and link spam.\n"
            "📡 <b>AutoLower Radar:</b> Automatically attenuates ambient mics down to 2% during voice chats.\n\n"
            "🛡️ <i>Cloud Media Management</i>"
        ),
        "info_mod_channels": (
            "📡 <b>Channels & Lives — Operations Guide</b>\n\n"
            "🎙️ <b>Live Sentinel:</b> Automatically opens a member's microphone upon 'Raise Hand'.\n"
            "🔗 <b>Paywalled Invites:</b> Generates single-use invite links that burn upon joining.\n"
            "🔁 <b>Recurring Audit:</b> Sends renewal alerts and executes auto-kick for unpaid members.\n\n"
            "🛡️ <i>Cloud Media Management</i>"
        ),
        "info_mod_monetization": (
            "💰 <b>Stars Monetization — Operations Guide</b>\n\n"
            "🎙️ <b>VIP Microphone Pass (/micvip):</b> Sells temporary 24-hour speaking privileges in voice chats.\n"
            "🧬 <b>Bot Clone Architecture:</b> Run an independent replica keeping 100% of the revenues.\n\n"
            "🛡️ <i>Cloud Media Management</i>"
        ),
        ...
        "pay_pro_title": "⭐ <b>PRO Plan Subscription — {group_name} (300 Stars)</b>\n\nUpgrade your community to elite operational status.",
        "pay_ultra_title": "💎 <b>ULTRA PRO Subscription — {group_name} (600 Stars)</b>\n\nTotal command, decentralized automation, and AI Sentinel tools.",
        ...
        "captcha_main_title": "🤖 <b>Captcha Protection (Captcha Pro)</b>",
        "captcha_action_title": "⚖️ <b>Punishment Configuration</b>",
        "antispam_main_title": "✉️ <b>Granular Anti-Spam Matrix</b>",
        "locks_main_title": "🔒 <b>Locks & Restrictions</b>",
        "warns_main_title": "⚠️ <b>Tactical Warnings (Warns Matrix)</b>",
        "delmsgs_main_title": "🗑️ <b>Advanced Message Purge Center</b>",
        "antiflood_main_title": "🗣️ <b>Anti-Flood Shield</b>",
        "forwards_panel": "🌊 <b>Forwards Shield Configuration</b>",
        "clone_main_title": "🧬 <b>Clone & Dedicated Sentinel Architecture</b>",
        ...
        "sched_err": "⚠️ Invalid format. Use HH:MM-HH:MM.",
        "wl_success": "✅ <b>User successfully whitelisted!</b>",
        "id_err": "⚠️ <b>Invalid ID.</b>",
        "bl_success": "✅ <b>Term blacklisted!</b>",
        ...
        "tag_updated": "🏷️ <b>VIP Tag Updated!</b>",
        ...
        "mic_alert_set": "Rate set successfully ⭐",
        ...
        "mic_menu": "🎙️ <b>VIP Microphone Pass</b>",
        "tag_menu_prompt": "🏷️ <b>Native VIP Tag Editor</b>:",
        "mod_ask_time": "⚡ <b>Moderation Directive</b>",
        "mod_ask_target": "🎯 <b>Target Configuration</b>",
        ...
        "btn_duck_level": "🎚️ Ducking Level",
        ...
        "btn_speakers_price": "⭐ Priority Rate",
    ...
    "es": {
        ...
        "info_main": (
            ...
            "<b>The Bunker Bot</b>\n"
            "• <b>Versión:</b> 6.2 (Centinela IA Core)\n"
        ...
        "info_mod_groups": (
            ...
            "🔐 <b>Aduana Captcha Pro:</b> Desafíos privados antes de ingresar.\n"
            "🔒 <b>Cerraduras Granulares:</b> Restringe multimedia, enlaces, stickers y comandos.\n"
            "🚫 <b>Escudo Anti-Spam:</b> Bloqueo automático de reenvíos y enlaces maliciosos.\n"
            "📡 <b>Radar AutoLower:</b> Atenúa micrófonos ambientales al 2% durante llamadas.\n\n"
            "🛡️ <i>Cloud Media Management</i>"
        ),
        "info_mod_channels": (
            "📡 <b>Canales y Lives — Guía Operativa</b>\n\n"
            "🎙️ <b>Centinela en Vivo:</b> Apertura automática de micrófono al levantar la mano.\n"
            "🔗 <b>Paywalled Invites:</b> Enlaces de un solo uso que se queman al entrar.\n"
            "🔁 <b>Auditoría Recurrente:</b> Alertas de renovación y auto-kick a morosos.\n\n"
            "🛡️ <i>Cloud Media Management</i>"
        ),
        "info_mod_monetization": (
            "💰 <b>Monetización Stars — Guía Operativa</b>\n\n"
            "🎙️ <b>Pase VIP de Micrófono (/micvip):</b> Venta de privilegios de voz por 24h.\n"
            "🧬 <b>Arquitectura de Bots Clones:</b> Opera tu réplica con 100% de ingresos para ti.\n\n"
            "🛡️ <i>Cloud Media Management</i>"
        ),
        ...
        "pay_pro_title": "⭐ <b>Suscripción Plan PRO — {group_name} (300 Stars)</b>\n\nEleva tu comunidad a un entorno operativo avanzado.",
        "pay_ultra_title": "💎 <b>Suscripción ULTRA PRO — {group_name} (600 Stars)</b>\n\nControl total, automatización descentralizada y herramientas de IA.",
        ...
        "mod_main": "🛡️ <b>Matriz de Moderación Táctica</b>\n\nConsola remota para <b>{group_name}</b>.",
        "eco_main": "📡 <b>Radar y Control del Ecosistema</b>\n\nTelemetría en tiempo real y supervisión para <b>{group_name}</b>.",
        ...
        "captcha_main_title": "🤖 <b>Protección Captcha (Captcha Pro)</b>",
        "captcha_action_title": "⚖️ <b>Configuración de Castigo</b>",
        "antispam_main_title": "✉️ <b>Matriz Anti-Spam Granular</b>",
        "locks_main_title": "🔒 <b>Panel de Cerraduras (Locks)</b>",
        "warns_main_title": "⚠️ <b>Matriz de Advertencias (Warns)</b>",
        "delmsgs_main_title": "🗑️ <b>Centro Avanzado de Purga de Mensajes</b>",
        "antiflood_main_title": "🗣️ <b>Escudo Anti-Flood</b>",
        "forwards_panel": "🌊 <b>Configuración del Escudo de Reenvíos</b>",
        "clone_main_title": "🧬 <b>Arquitectura de Clonación & Centinela Dedicado</b>",
        "botfather_guide": "🔑 <b>Conectar Bot Clon:</b> Envía <code>/newbot</code> a @BotFather y pega el token aquí.",
        "sentinel_phone_guide": "🎙️ <b>Conectar Centinela:</b> Envía tu número telefónico con código internacional.",
        ...
        "sched_err": "⚠️ Formato incorrecto. Usa HH:MM-HH:MM.",
        "wl_success": "✅ <b>¡Usuario registrado en Whitelist!</b>",
        "id_err": "⚠️ <b>ID inválido.</b>",
        "bl_success": "✅ <b>¡Término prohibido registrado en Blacklist!</b>",
        "dir_ban": "🚫 <b>¡Baneo ejecutado remotamente!</b>",
        "dir_kick": "👢 <b>¡Expulsión ejecutada remotamente!</b>",
        "dir_mute": "🔇 <b>¡Silencio ejecutado remotamente!</b>",
        "dir_unmute": "🔊 <b>¡Permisos restaurados!</b>",
        "dir_err": "❌ <b>Error al ejecutar directiva:</b> {ex}",
        "mic_updated": "⭐ <b>¡Tarifa VIP actualizada!</b>",
        "mic_err": "⚠️ Ingresa un número entero positivo.",
        "tag_updated": "🏷️ <b>¡Etiqueta VIP actualizada!</b>",
        ...
        "mic_alert_set": "Tarifa configurada con éxito ⭐",
        ...
        "mic_menu": "🎙️ <b>Pase VIP de Micrófono</b>",
        "tag_menu_prompt": "🏷️ <b>Editor de Etiqueta VIP Nativa</b>:",
        "mod_ask_time": "⚡ <b>Directiva de Moderación</b>",
        "mod_ask_target": "🎯 <b>Configuración de Objetivo</b>",
        ...
        "vcsched_prompt": "⏰ <b>Configuración Horario VC</b>",
        ...
        "btn_mictag": "🏷️ Etiqueta",
        "btn_sched_mod": "⏰ Modificar Horario",
        ...
        "btn_duck_level": "🎚️ Nivel de Ducking",
        "duck_custom_prompt": "🎚️ <b>Nivel de Ducking Personalizado (1-90):</b>",
        "duck_updated": "🎚️ <b>¡Ducking actualizado!</b>",
        ...
        "btn_speakers_price": "⭐ Tarifa Prioridad",
        ...
        "night_main": "🏴‍☠️ <b>Modo Nocturno Autónomo (Fase 4)</b>",
        "night_prompt": "⏰ <b>Configuración de Horario Nocturno</b>",
        "night_updated": "✅ <b>¡Horario del Modo Nocturno actualizado con éxito!</b>",
        "night_err": "⚠️ Formato incorrecto. Usa HH:MM-HH:MM.",
        "btn_night_on": "🟢 Activar Modo Nocturno",
        "btn_night_off": "🔴 Desactivar Modo Nocturno",
        "btn_night_mod": "⏰ Modificar Horario"
    }
}


def ultra_back_button(chat_type: str, group_id: int, lang: str) -> InlineKeyboardButton:
    """Build the back button for the current group or channel context."""
    t = TEXTS.get(lang, TEXTS["es"])
    if chat_type == "c":
        return InlineKeyboardButton(
            text=t["btn_back_channel"],
            callback_data=f"cpanel_{group_id}_{lang}"
        )
    return InlineKeyboardButton(
        text=t["btn_back_group"],
        callback_data=f"gpanel_{group_id}_{lang}"
    )


def _cancel_kb(t: dict, callback_data: str) -> InlineKeyboardMarkup:
    """Build the cancel keyboard used by conversational setup prompts."""
...
def get_ultra_tools_keyboard(group_id: int, lang: str, chat_type: str = "g"):
    """Build the ULTRA tools keyboard for the current group or channel context."""
    t = TEXTS.get(lang, TEXTS["es"])
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=t["panic_menu"], callback_data=f"panic_menu_{group_id}_{lang}")],
        [InlineKeyboardButton(text=t["shield_menu"], callback_data=f"shield_menu_{group_id}_{lang}")],
        [InlineKeyboardButton(text=t["podcast_menu"], callback_data=f"podcast_menu_{group_id}_{lang}")],
        [InlineKeyboardButton(text=t["speakers_menu"], callback_data=f"speakers_menu_{group_id}_{lang}")],
        [InlineKeyboardButton(text=t["btn_sentinel_payload"], callback_data=f"payload_menu_{group_id}_{lang}")],
        [ultra_back_button(chat_type, group_id, lang)],
    ])
...
async def get_clone_keyboard(group_id: int, user_id: int, lang: str, chat_type: str = "g"):
    ...
    if tier == "ultra_pro":
        ...
        token_btn_text = (
            ("🔄 Actualizar Token @BotFather" if lang == "es" else "🔄 Update @BotFather Token") if has_clone
            else ("🔑 Conectar Token @BotFather" if lang == "es" else "🔑 Connect @BotFather Token")
        )
        sentinel_btn_text = (
            ("🔄 Actualizar Centinela (Teléfono) 🟢" if lang == "es" else "🔄 Update Sentinel (Phone) 🟢") if has_sentinel
            else ("🎙️ Conectar Centinela Propio (Teléfono) 🔴" if lang == "es" else "🎙️ Connect Own Sentinel (Phone) 🔴")
        )
        ...
        if has_clone:
            kb.append([InlineKeyboardButton(text="🛑 Desconectar Bot Clon" if lang == "es" else "🛑 Disconnect Clone Bot", callback_data=f"clone_discbot_{group_id}_{lang}")])
        if has_sentinel:
            kb.append([InlineKeyboardButton(text="🛑 Desconectar Centinela" if lang == "es" else "🛑 Disconnect Sentinel", callback_data=f"clone_discsentinel_{group_id}_{lang}")])
...
def get_channel_perm_warning(bot, group_id, lang):
    raise NotImplementedError


def get_info_keyboard(lang: str):
    """Build the information menu keyboard for the selected language."""
    t = TEXTS.get(lang, TEXTS["es"])
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=t["btn_how_works"],
            callback_data=f"menu_infohow_{lang}",
        )],
        [InlineKeyboardButton(
            text=t["btn_back"],
            callback_data=f"menu_main_{lang}",
        )],
    ])


def get_group_panel_keyboard(group_id: int, lang: str):
    """Build the main inline keyboard for a group's management panel."""
    t = TEXTS.get(lang, TEXTS["es"])
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=t["btn_mod"], callback_data=f"menu_mod_{group_id}_{lang}")],
        [InlineKeyboardButton(text=t["btn_eco"], callback_data=f"menu_eco_{group_id}_{lang}")],
        [InlineKeyboardButton(text=t["btn_antispam"], callback_data=f"gset_antispam_{group_id}_{lang}")],
        [InlineKeyboardButton(text=t["btn_antiflood"], callback_data=f"gset_antiflood_{group_id}_{lang}")],
        [InlineKeyboardButton(text=t["btn_captcha"], callback_data=f"gset_captcha_{group_id}_{lang}")],
        [InlineKeyboardButton(text=t["btn_locks"], callback_data=f"gset_locks_{group_id}_{lang}")],
        [InlineKeyboardButton(text=t["btn_warns"], callback_data=f"gset_warns_{group_id}_{lang}")],
        [InlineKeyboardButton(text=t["btn_delmsgs"], callback_data=f"gset_delmsgs_{group_id}_{lang}")],
...
def get_channel_panel_keyboard(group_id: int, lang: str):
    """Return the inline keyboard for a channel's private management panel."""
    t = TEXTS.get(lang, TEXTS["es"])
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=t["btn_tips"],
        ...
        [InlineKeyboardButton(
            text=t["btn_sentinel_payload"],
            callback_data=f"payload_menu_{group_id}_{lang}"
        )],
        [InlineKeyboardButton(
            text=t["btn_night_mode"],
            callback_data=f"night_menu_{group_id}_{lang}"
        )],
        [InlineKeyboardButton(
            text=t["btn_back_chsettings"],
            callback_data=f"chsettings_{lang}"
        )]
    ])


def send_official_welcome(bot, chat_id, user, bot_username):
    raise NotImplementedError
...
async def cmd_cancel(message: Message, bot: Bot):
    ...
    await message.answer(f"✅ {t['op_canceled']}", reply_markup=kb, parse_mode="HTML")
...
def origin_back_button(bot, group_id, lang):
    raise NotImplementedError

def get_channels_keyboard(channels, lang):
    """Build the inline keyboard for the user's registered channels."""
    t = TEXTS.get(lang, TEXTS["es"])
    rows = []
    for channel in channels or []:
        if isinstance(channel, dict):
            channel_id = channel.get("channel_id", channel.get("id"))
            title = channel.get("title") or str(channel_id)
        elif isinstance(channel, (tuple, list)):
            channel_id = channel[0] if channel else None
            title = channel[1] if len(channel) > 1 else str(channel_id)
        else:
...
async def get_active_user_channels(bot, user_id):
    """Return registered channels that are still accessible to the bot."""
...
async def handle_private_inputs(message: Message, bot: Bot):
    ...
    if (bot.id, user_id) in AI_PROMPT_STATES:
        ...
        if text_input:
            ...
            empty_prompt_msg = (
                "⚠️ El prompt no puede estar vacío."
                if lang == "es" else
    ...
    if (bot.id, user_id) in CAPTCHA_STATES:
        ...
        if not text_input:
            ...
            resp = await message.answer(
                ("⚠️ Envía el mensaje del captcha como <b>texto</b>." if lang == "es" else "⚠️ Send the captcha message as <b>text</b>.") + "\n\n🛡️ <i>Cloud Media Management</i>",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                    InlineKeyboardButton(
                        text=t.get("btn_back_captcha", "🔙 Volver"),
                        callback_data=f"gset_captcha_{group_id}_{lang}"
                    )
                ]]), parse_mode="HTML"
            )
            fire_and_forget_auto_delete([message, resp], delay=60)
        ...
        try:
            ...
            resp = await message.answer(
                tr(lang, "⚠️ No pude guardar el mensaje del captcha.", "⚠️ I couldn't save the captcha message.") + "\n\n🛡️ <i>Cloud Media Management</i>",
                reply_markup=back_kb, parse_mode="HTML"
            )
        fire_and_forget_auto_delete([message, resp], delay=60)
        return
...
def build_channel_request_keyboard(lang):
    raise NotImplementedError

def get_active_user_groups():
    raise NotImplementedError
...
async def process_menu_navigation(callback: CallbackQuery, bot: Bot):
    ...
    if action == "lang":
        ...
        if sub_cmd == "wl":
            ...
            status_str = ("🟢 ACTIVADO (2% para no autorizados)" if lang == "es" else "🟢 ACTIVE (2% for unauthorized)") if curr_al == 1 else ("🔴 DESACTIVADO (Micrófonos Libres)" if lang == "es" else "🔴 DISABLED (Open Mics)")
...
def safe_edit_text(callback, text, reply_markup=None, parse_mode=None):
    raise NotImplementedError
...
async def cb_group_modules_interceptor(callback: CallbackQuery, bot: Bot):
    ...
    if action == "gset":
        ...
        prompt = (
            f"<b>{t['af_msgs']}</b>\n"
            f"{tr(lang, 'Selecciona el límite:', 'Select the limit:')}"
            "\n🛡️ <i>Cloud Media Management</i>"
        )
...
def build_ultra_lock_view(group_id, lang, feature_title, chat_type):
    raise NotImplementedError
...
async def cb_channel_plans_dispatch(callback: CallbackQuery, bot: Bot):
    ...
    if sub == "menu":
        ...
        # The persistence helper is optional in deployments where plan-state
        # storage is not configured; keep the in-memory flow functional there.
